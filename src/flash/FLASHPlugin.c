...
static int call_plugin_func(struct target *target, int timeout, uint32_t function, uint32_t sp, int32_t *result,
                            int argc, ...);
...
static void report_flash_progress(const char *op, uint32_t region_start, uint32_t region_end, const char *block_name)
{
    LOG_INFO("%s: 0x%x | 0x%x | %s", op, region_start, region_end, block_name);
}

int save_region(struct target *target, struct memory_backup *region, Elf32_Addr sh_addr, Elf32_Size sh_size,
                int sectionNumber)
{
    ...
    return target_read_memory(target, region->base_address, 4, (region->size + 3) / 4,
                              (uint8_t *)region->original_contents);
}
...
static int loaded_plugin_load(struct target *target, struct advanced_elf_image *image, struct loaded_plugin *plugin,
                              uint32_t init_done_address, unsigned stackSize)
{
    int retval;
    ...
    if (retval == ERROR_OK) {
        ...
        for (int i = 0; i < plugin->region_count; i++) {
            if (plugin->regions[i].original_section < 0 ||
                image->sections[plugin->regions[i].original_section].sh_type == 8 /* NOBITS */)
                continue;

            size_t done;
            retval = advanced_elf_image_read_section(image, plugin->regions[i].original_section, (uint8_t *)pBuf,
                                                     plugin->regions[i].size, &done);
            if (retval != ERROR_OK)
                break;
            ...
            if (timeoutTable && timeoutTable >= plugin->regions[i].base_address &&
                timeoutTable < (plugin->regions[i].base_address + plugin->regions[i].size)) {
                ...
                if (timeout_struct_size > sizeof(struct plugin_timeouts))
                    ...
                    memcpy(&plugin->timeouts, &((struct image_plugin_timeouts *)timeouts_from_image)->timeouts,
                           timeout_struct_size);
            }

            retval = target_write_memory(target, plugin->regions[i].base_address, 4, (plugin->regions[i].size + 3) / 4,
                                         (uint8_t *)pBuf);
            if (retval != ERROR_OK)
                break;
        }
    ...
    if (retval == ERROR_OK && init_done_address) {
        ...
        retval = target_run_algorithm(target, 0, NULL, sizeof(reg_params) / sizeof(reg_params[0]), reg_params,
                                      plugin->entry, init_done_address, plugin->timeouts.load, &armv7m_info);
        breakpoint_remove(target, init_done_address);
        if (retval != ERROR_OK)
            LOG_ERROR(
                "FLASH plugin did not call FLASHPlugin_InitDone(). Ensure it is declared as non-inline and try increasing load timeout setting inside the plugin timeout table.");
...
static int loaded_plugin_unload(struct loaded_plugin *plugin, uint32_t unload_func)
{
    ...
    for (int i = 0; i < plugin->region_count; i++) {
        ...
        int r = target_write_memory(plugin->target, plugin->regions[i].base_address, 4,
                                    (plugin->regions[i].size + 3) / 4, (uint8_t *)plugin->regions[i].original_contents);
    ...
    if (plugin->work_area_backup.size && plugin->work_area_backup.original_contents) {
        int r = target_write_memory(plugin->target, plugin->work_area_backup.base_address, 4,
                                    plugin->work_area_backup.size / 4, plugin->work_area_backup.original_contents);
...
static int loaded_plugin_backup_workarea(struct loaded_plugin *plugin, uint32_t start, uint32_t size)
{
    ...
    int retval = target_read_memory(plugin->target, start + baseoff, 4, size / 4,
                                    ((uint8_t *)plugin->work_area_backup.original_contents) + baseoff);
    if (retval != ERROR_OK)
        return retval;
...
FLASH_BANK_COMMAND_HANDLER(plugin_flash_bank_command)
{
    ...
    if (!info->FLASHPlugin_ProgramAsync && !info->FLASHPlugin_ProgramSync) {
        LOG_ERROR(
            "%s: invalid FLASH plugin. Neither 'FLASHPlugin_ProgramSync' nor 'FLASHPlugin_ProgramAsync' is defined.",
            URL);
        return ERROR_IMAGE_FORMAT_ERROR;
    }

    info->FLASHPlugin_CheckSectorProtection =
        advanced_elf_image_find_symbol(&info->image, "FLASHPlugin_CheckSectorProtection");
...
int plugin_write_sync(struct target *target, struct plugin_flash_bank *plugin_info, struct loaded_plugin *loaded_plugin,
                      uint32_t offset, const uint8_t *buffer, uint32_t size)
{
    ...
    if (todo & 3) {
        retval =
            target_write_memory(target, plugin_info->WorkArea.Address + (todo & ~3), 1, todo & 3, buffer + (todo & ~3));
        if (retval != ERROR_OK)
            return -1;
    }

    int32_t result = 0;
    retval = call_plugin_func(target, loaded_plugin->timeouts.write, plugin_info->FLASHPlugin_ProgramSync,
                              loaded_plugin->sp, &result, 3, offset, plugin_info->WorkArea.Address, todo);
    if (retval != ERROR_OK)
        return -1;
    return result;
}

int plugin_write_async(struct target *target, struct plugin_flash_bank *plugin_info,
                       struct loaded_plugin *loaded_plugin, uint32_t offset, const uint8_t *buffer, uint32_t size)
{
    const int fifo_header_size = 8;
    unsigned workAreaSize =
        MIN(MAX(size, plugin_info->write_block_size) + plugin_info->write_block_size + fifo_header_size,
            plugin_info->WorkArea.Size);
    ...
    if (workAreaSize > plugin_info->WorkArea.Size) {
        LOG_ERROR("Computed worka area size (0x%x) is smaller then the available size (0x%x)", workAreaSize,
                  plugin_info->WorkArea.Size);
    ...
    retval = target_run_flash_async_algorithm(
        target, buffer, (size + plugin_info->write_block_size - 1) / plugin_info->write_block_size,
        plugin_info->write_block_size, 0, NULL, sizeof(reg_params) / sizeof(reg_params[0]), reg_params,
        plugin_info->WorkArea.Address, workAreaSize, plugin_info->FLASHPlugin_ProgramAsync, return_addr, &armv7m_info);
...
static int plugin_write(struct flash_bank *bank, const uint8_t *buffer, uint32_t offset, uint32_t count)
{
    struct target *target = bank->target;
    struct plugin_flash_bank *plugin_info = bank->driver_priv;
    struct loaded_plugin loaded_plugin;

    int retval = loaded_plugin_load(target, &plugin_info->image, &loaded_plugin, plugin_info->FLASHPlugin_InitDone,
                                    plugin_info->stack_size);
    if (retval == ERROR_OK) {
        if (plugin_info->FLASHPlugin_ProgramAsync &&
            plugin_info->FLASHPlugin_ProgramAsync != plugin_info->FLASHPlugin_NotImplemented)
            retval = plugin_write_async(target, plugin_info, &loaded_plugin, offset, buffer, count);
        else if (plugin_info->FLASHPlugin_ProgramSync &&
                 plugin_info->FLASHPlugin_ProgramSync != plugin_info->FLASHPlugin_NotImplemented) {
            uint32_t done = 0;
            while (done < count) {
                int doneNow =
                    plugin_write_sync(target, plugin_info, &loaded_plugin, offset + done, buffer + done, count - done);
                if (doneNow < 0) {
                    retval = ERROR_FLASH_BANK_INVALID;
                    break;
                }

                report_flash_progress("flash_write_progress_sync", bank->base + offset + done,
                                      bank->base + offset + done + doneNow, bank->name);

                done += doneNow;
            }
        } else {
            LOG_ERROR(
                "Neither FLASHPlugin_ProgramAsync() or FLASHPlugin_ProgramSync() are properly defined in the FLASH plugin. Cannot program FLASH memory.");
            retval = ERROR_FLASH_BANK_INVALID;
        }
    }
                            ...
                            int argc, ...)
{
    ...
    int retval = target_run_algorithm(target, 0, NULL, reg_param_count, reg_params, function, return_addr, timeout,
                                      &armv7m_info);
    breakpoint_remove(target, return_addr);
...
static int plugin_probe(struct flash_bank *bank)
{
    ...
    if (retval == ERROR_OK) {
        ...
        retval = call_plugin_func(target, loaded_plugin.timeouts.init, plugin_info->FLASHPlugin_Probe, sp, &result, 5,
                                  sp, (uint32_t)bank->base, (uint32_t)bank->size, (uint32_t)bank->chip_width,
                                  (uint32_t)bank->bus_width);
        if (retval == ERROR_OK) {
    ...
    if (plugin_info->FLASHPlugin_FindWorkArea && retval == ERROR_OK) {
        ...
        retval = call_plugin_func(target, loaded_plugin.timeouts.init, plugin_info->FLASHPlugin_FindWorkArea, sp,
                                  &result, (uint32_t)2, sp, loaded_plugin.sp);
        if (retval == ERROR_OK) {
            retval =
                target_read_memory(target, sp, 4, sizeof(plugin_info->WorkArea) / 4, (uint8_t *)&plugin_info->WorkArea);
        }
    }
...
static int plugin_erase(struct flash_bank *bank, unsigned first, unsigned last)
{
    ...
    if (retval == ERROR_OK) {
        while (first <= last) {
            int32_t result;
            retval = call_plugin_func(target, loaded_plugin.timeouts.erase, plugin_info->FLASHPlugin_EraseSectors,
                                      loaded_plugin.sp, &result, 2, (uint32_t)first,
                                      (uint32_t)last - (uint32_t)first + 1);
            ...
            report_flash_progress("flash_erase_progress", bank->base + bank->sectors[first].offset,
                                  bank->base + bank->sectors[first + result - 1].offset +
                                      bank->sectors[first + result - 1].size,
                                  bank->name);
            first += result;
        }
    }
...
static int plugin_protect(struct flash_bank *bank, int set, unsigned first, unsigned last)
{
    ...
    if (!plugin_info->FLASHPlugin_ProtectSectors ||
        plugin_info->FLASHPlugin_ProtectSectors == plugin_info->FLASHPlugin_NotImplemented)
        return ERROR_OK; //Nothing to do
    ...
    if (retval == ERROR_OK) {
        while (first <= last) {
            int32_t result;
            retval = call_plugin_func(target, loaded_plugin.timeouts.protect, plugin_info->FLASHPlugin_ProtectSectors,
                                      loaded_plugin.sp, &result, 3, (uint32_t)set, (uint32_t)first,
                                      (uint32_t)last - (uint32_t)first + 1);
            if (retval != ERROR_OK)
                break;
            if (result < 0) {
...
static int plugin_protect_check(struct flash_bank *bank)
{
    ...
    if (!plugin_info->FLASHPlugin_CheckSectorProtection ||
        plugin_info->FLASHPlugin_CheckSectorProtection == plugin_info->FLASHPlugin_NotImplemented)
        return ERROR_OK; //Nothing to do
    ...
    if (retval == ERROR_OK) {
        ...
        if (retval == ERROR_OK) {
            for (unsigned sector = 0; sector < bank->num_sectors;) {
                ...
                int32_t result;
                retval = call_plugin_func(target, loaded_plugin.timeouts.protect,
                                          plugin_info->FLASHPlugin_CheckSectorProtection, loaded_plugin.sp, &result, 3,
                                          sector, sectors_to_check, plugin_info->WorkArea.Address);
                if (retval != ERROR_OK)
                    break;
